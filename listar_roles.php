...
<body class="cbp-spmenu-push">
  <div class="main-content">
    ...
    <div id="page-wrapper">
      <div class="main-page">
        <div class="tables">
          <h2 class="title1">ROLES ACTIVOSSS</h2>
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form action="" id="form_registro_paises">
                    ...
                    <br>
                    <input class="form-control" name="nombre_rol" id="nombre_rol">
                    <br>
                    <label>Descripción</label>
                    <br>
                    <input class="form-control" name="descripcion_rol" id="descripcion_rol">
                    <br>
                    <button type="button" onclick="guardar_rol()" class="btn btn-success">Guardar Rol</button>
                  </form>
                  <div id="message"></div>
                  <script>
                   function guardar_rol() {
                      const nombre_rol = document.getElementById('nombre_rol').value;
                      const descripcion_rol = document.getElementById('descripcion_rol').value;
                    
                      const xhr = new XMLHttpRequest();
                      xhr.open('POST', 'ajax_guardar_roles.php', true);
                      ...
                      xhr.onreadystatechange = function() {
                        if (xhr.readyState === 4 && xhr.status === 200) {
                          ...
                          if (xhr.responseText.includes("guardado exitosamente")) {
                            limpiarModal(); // Call function to clear modal
                            refreshTable(); // Call function to refresh table after saving
                          }
                        }
                      };
                    
                      const data = `nombre_rol=${encodeURIComponent(nombre_rol)}&descripcion_rol=${encodeURIComponent(descripcion_rol)}`;
                      xhr.send(data);
                    }

                    function refreshTable() {
                      const xhr = new XMLHttpRequest();
                      xhr.open('GET', 'ajax_actualiza_tabla_roles.php', true); // Archivo PHP que genera la tabla de países
                      xhr.onreadystatechange = function() {
                        if (xhr.readyState === 4 && xhr.status === 200) {
          ...
          <div class="bs-example widget-shadow" data-example-id="hoverable-table">
                  ...
                  <tr>
                    ...
                    <td>
                      <div class="button-container">
                        <form action="controller/roles/controller.php" method="POST">
                        ...
                        <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#exampleModalE_<?php echo $extraer_roles['identificador']; ?>">
                          <i class="fa fa-edit"></i> Editar
                        </button>
                        <div class="modal fade" id="exampleModalE_<?php echo $extraer_roles['identificador']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabelE_<?php echo $extraer_roles['identificador']; ?>" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabelE_<?php echo $extraer_roles['identificador']; ?>">Actualizar Rol</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body">
                                        <form id="form_editar_rol_<?php echo $extraer_roles['identificador']; ?>">
                                            <input type="hidden" name="id_pais" value="<?php echo $extraer_roles['identificador']; ?>">
                                            <label for="nombre_pais">Nombre Rol</label>
                                            <input type="text" class="form-control" id="nombre_rol_<?php echo $extraer_paises['identificador']; ?>" name="nombre_rol" value="<?php echo $extraer_paises['nombre_pais']; ?>">
                                        </form>
                                        <div id="message_<?php echo $extraer_roles['identificador']; ?>"></div>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
                                        <button type="button" class="btn btn-primary" onclick="actualizar_rol_<?php echo $extraer_roles['identificador']; ?>()">Guardar cambios</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <script>
                            ...
                            $('#exampleModalE_<?php echo $extraer_roles['identificador']; ?>').on('hidden.bs.modal', function () {
                                refreshTable();
                            });
                        </script>
                      </div>
            ...
            <script>
              <?php
              $consulta_roles->data_seek(0);
              while ($extraer_roles = $consulta_roles->fetch_array()) {
              ?>

                function actualizar_roles_<?php echo $extraer_roles['identificador']; ?>() {
            ...
            <script>
              function refreshTable() {
                    const xhr = new XMLHttpRequest();
                    xhr.open('GET', 'ajax_actualiza_tabla_roles.php', true);
                    xhr.onreadystatechange = function () {
                        if (xhr.readyState === 4 && xhr.status === 200) {
                            document.getElementById('dataTable').innerHTML = xhr.responseText;
